...
class GaterSIMO(DivisionStepNeuralTangent):
    ...
    def gating_function(self, testdataset):
        """ Given the test_jacobian, we compute its expert number i,
        for which there will be a GP to predict.

        Args:
            test_jacobian ([type]): [description]
            targetout ([type]): [description]
        ...
        for targetout in range(self.outdim):
            ...
            # pca and boundary points
            bdata = self.divisionstep(None, testdataset)
            test_bdatalist.append(bdata)
        return test_bdatalist
