""" Utility scripts. Codes taken from other repositories with modifications.
"""
import torch
import functools
...
def d1_to_d2(A):
    '''
    if 1d array, we convert into 2d with an empty dim
    '''
    if A.ndim == 1:
...
def tprep(A):
    '''
    model quantiles expects 1, 1, 1 ndim = 2
    '''
    length_t = list(A.size())
    if len(length_t) == 2:
        return A.unsqueeze(-1)
    elif len(length_t) == 1:
        return A.unsqueeze(-1).unsqueeze(-1)
    else:
        return A


def read_dropout_tau(dir):
    with open(dir, "r") as f:
        lines = f.readlines()
        tau = float(lines[2][lines[2].index(":")+2: -1])
        dropout_rate = float(lines[3][lines[3].index(":")+2: -1])
    return dropout_rate, tau